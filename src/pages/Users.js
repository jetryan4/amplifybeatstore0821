...
import {createConnectedAccount, updateUser} from "../api/mutations";
...
const Users = () => {
    ...
    const promoteUser = async (userId, email) => {
        try {
            ...
            if (res.message) {
                ...
                await API.graphql({
                    query: updateUser,
                    variables: {input: {id: userId, role: 'Producer'}},
                    authMode: "AMAZON_COGNITO_USER_POOLS"
                });
                // Send 'create connected account' link if connected account is not created or details are not submitted
                const userDetails = users.filter((user) => user.id === userId)[0];
                if (!userDetails.stripeAccount || userDetails.stripeAccountStatus !== 'details_submitted') {
                    const payload = {
                        userId, email
                    };
                    await API.graphql(graphqlOperation(createConnectedAccount, {input: payload}));
                }
                window.location.reload();
            }
            console.log("add to group is successful", JSON.stringify(res));
        } catch (err) {
            console.log(err);
        }
    };
    ...
    return (<div className=" py-10 md:ml-20 sm:ml-16 xl:mr-4">
                ...
                <div className="py-2 align-middle inline-block min-w-full sm:px-6">
                    <div className="shadow  border-b border-gray-200 sm:rounded-lg xl:mr-8">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            {users.map((person) => (<tr key={person.email}>
                        ...
                        {person.status}
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => promoteUser(person.id,person.email)}
                                        >
                                            Promote
                                        </button>
                                        &nbsp;
