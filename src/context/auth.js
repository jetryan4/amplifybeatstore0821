...
const reducer = (state, action) => {
    switch (action.type) {
        case "LOGIN_REQUEST":
            return {
                ...state, isLoggedIn: false, user: null, isLoggingIn: true
            };
        case "LOGIN_SUCCESS":
            return {
                ...state, isLoggedIn: true, user: action.payload.user, isLoggingIn: false
            };
        case "LOGIN_FAILURE":
            return {
                ...state, isLoggedIn: false, user: null, isLoggingIn: false
            };
        case "LOGOUT_SUCCESS":
            return {
                ...state, ...initialState
...
const AuthProvider = ({children}) => {
    ...
    const persistedUserState = {
        ...initialState, user: persistedUser, isLoggedIn: _get(persistedUser, "username", "").length > 0
    };
